...
import type { NewsSkeleton, ProcessedNews, EventSkeleton, ProcessedEvent, NewsletterSkeleton, ProcessedNewsletter, ResearchSkeleton, ProcessedResearch, VoiceSkeleton, ProcessedVoice } from './contentful-types'
...
export async function getAllNews(preview = false): Promise<Entry<NewsSkeleton>[]> {
  return getEntries<NewsSkeleton>('news', { order: ['-fields.date'] }, preview)
}

export async function getNewsByCategory(category: string, preview = false): Promise<Entry<NewsSkeleton>[]> {
  try {
    const client = getClient(preview)
    const query: any = {
      content_type: 'news',
...
export async function getFeaturedNews(limit = 3, preview = false): Promise<Entry<NewsSkeleton>[]> {
  return getEntries<NewsSkeleton>('news', { 
    order: ['-fields.date'],
    limit 
  }, preview)
}

export async function getNewsById(entryId: string, preview = false): Promise<Entry<NewsSkeleton> | null> {
...
export async function getAllEvents(preview = false): Promise<Entry<EventSkeleton>[]> {
  return getEntries<EventSkeleton>('event', { order: ['fields.startDate'] }, preview)
}

export async function getUpcomingEvents(preview = false): Promise<Entry<EventSkeleton>[]> {
  try {
    const client = getClient(preview)
...
export async function getPastEvents(preview = false): Promise<Entry<EventSkeleton>[]> {
  try {
    const client = getClient(preview)
    const now = new Date().toISOString()
    const query: any = {
...
export async function getEventById(entryId: string, preview = false): Promise<Entry<EventSkeleton> | null> {
...
export function transformEvent(entry: Entry<EventSkeleton>): ProcessedEvent {
  ...
  // Format dates
  const startDate = fields.startDate ? new Date(fields.startDate as string) : null
  ...
  if (startDate) {
    if (endDate && startDate.toDateString() !== endDate.toDateString()) {
      // Multi-day event
      dateString = `${formatDate(fields.startDate as string)} - ${formatDate(fields.endDate as string)}`
...
export async function getAllNewsletters(preview = false): Promise<Entry<NewsletterSkeleton>[]> {
  return getEntries<NewsletterSkeleton>('newsletter', { order: ['-fields.date'] }, preview)
}

export async function getNewslettersByCategory(category: string, preview = false): Promise<Entry<NewsletterSkeleton>[]> {
  try {
    const client = getClient(preview)
    const query: any = {
...
export async function getLatestNewsletters(limit = 5, preview = false): Promise<Entry<NewsletterSkeleton>[]> {
  return getEntries<NewsletterSkeleton>('newsletter', { 
    order: ['-fields.date'],
    limit 
  }, preview)
}

export async function getNewsletterById(entryId: string, preview = false): Promise<Entry<NewsletterSkeleton> | null> {
  return getEntry<NewsletterSkeleton>(entryId, preview)
}

// Transform Contentful newsletter entry to processed format
export function transformNewsletter(entry: Entry<NewsletterSkeleton>): ProcessedNewsletter {
  const { fields, sys } = entry
  ...
  return {
    ...
    href: fileUrl && fileUrl !== '/placeholder.svg' ? fileUrl : `/resources/newsletter/${sys.id}`,
...
function determineNewsletterBadge(dateString: string): string {
  ...
  const now = new Date()
  const monthsDiff = (now.getFullYear() - newsletterDate.getFullYear()) * 12 + 
...
export async function getAllResearch(preview = false): Promise<Entry<ResearchSkeleton>[]> {
  return getEntries<ResearchSkeleton>('research', { order: ['-fields.date'] }, preview)
}

export async function getResearchByCategory(category: string, preview = false): Promise<Entry<ResearchSkeleton>[]> {
  try {
    const client = getClient(preview)
    const query: any = {
...
export async function getLatestResearch(limit = 5, preview = false): Promise<Entry<ResearchSkeleton>[]> {
  return getEntries<ResearchSkeleton>('research', { 
    order: ['-fields.date'],
    limit 
  }, preview)
}

export async function getResearchById(entryId: string, preview = false): Promise<Entry<ResearchSkeleton> | null> {
  return getEntry<ResearchSkeleton>(entryId, preview)
}

// Transform Contentful research entry to processed format
export function transformResearch(entry: Entry<ResearchSkeleton>): ProcessedResearch {
  const { fields, sys } = entry
  ...
  return {
    id: sys.id,
    title: (fields.title as string) || '',
    description: generateResearchDescription(fields.title as string, fields.category as string),
    date: formatResearchDate(fields.date as string),
    image: getAssetUrl(fields.thumbnail as Asset),
    badge: determineResearchBadge(fields.date as string, fields.category as string),
    downloadUrl: fileUrl,
...
function determineResearchBadge(dateString: string, category?: string): string {
  ...
  const now = new Date()
  const monthsDiff = (now.getFullYear() - researchDate.getFullYear()) * 12 + 
...
export async function getAllVoices(preview = false): Promise<Entry<VoiceSkeleton>[]> {
  return getEntries<VoiceSkeleton>('voice', { order: ['-fields.date'] }, preview)
}

export async function getFeaturedVoices(preview = false): Promise<Entry<VoiceSkeleton>[]> {
  try {
    const client = getClient(preview)
    const query: any = {
      content_type: 'voice',
...
export async function getLatestVoices(limit = 5, preview = false): Promise<Entry<VoiceSkeleton>[]> {
  return getEntries<VoiceSkeleton>('voice', { 
    order: ['-fields.date'],
    limit 
  }, preview)
}

export async function getVoiceById(entryId: string, preview = false): Promise<Entry<VoiceSkeleton> | null> {
...
export function transformVoice(entry: Entry<VoiceSkeleton>): ProcessedVoice {
  ...
  return {
    id: sys.id,
    title: (fields.title as string) || '',
    description: (fields.shortDescription as string) || generateVoiceDescription(fields.title as string),
    date: formatVoiceDate(fields.date as string),
    image: getAssetUrl(fields.thumbnail as Asset),
    badge: determineVoiceBadge(fields.date as string, fields.featured as boolean),
    href: fields.videoLinkYoutube ? (fields.videoLinkYoutube as string) : `/resources/voices/${sys.id}`,
    type: "video" as const,
...
function determineVoiceBadge(dateString: string, featured: boolean): string {
  ...
  const now = new Date()
  const monthsDiff = (now.getFullYear() - voiceDate.getFullYear()) * 12 + 
...
function determineVoiceCategory(title: string): string {
  ...
  if (titleLower.includes('mother') || titleLower.includes('parent')) return 'Community Stories'
  if (titleLower.includes('teacher') || titleLower.includes('training')) return 'Professional Development'
  if (titleLower.includes('children') || titleLower.includes('child')) return "Children's Perspectives"
  if (titleLower.includes('practice') || titleLower.includes('best')) return 'Best Practices'
  if (titleLower.includes('policy') || titleLower.includes('advocacy')) return 'Policy & Advocacy'
  if (titleLower.includes('research') || titleLower.includes('study')) return 'Research Insights'
